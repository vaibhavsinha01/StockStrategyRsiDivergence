...
class Strategy:
    ...
    def Divergence(self):
        ...
        for i in range(2, len(self.data) - 1):
            ...
            if (close < prev_close) & (rsi > prev_rsi):
                self.data['DivergenceSignal'].iloc[i] = 1
            elif (close > prev_close) & (rsi < prev_rsi):
                self.data['DivergenceSignal'].iloc[i] = -1
        return self.data
    ...
    def Signal(self):
        ...
        for i in range(len(self.data)):
            if (
                (self.data['Rsi'].iloc[i] < 40) &
                (self.data['Close'].iloc[i] < self.data['BBmid'].iloc[i] / 2) &
                (self.data['minterm'].iloc[i]) &
                (self.data['mintermr'].iloc[i]) 
            ):
                self.data['Signal'].iloc[i] = 1
            elif (
                (self.data['Rsi'].iloc[i] > 60) &
                (self.data['Close'].iloc[i] > self.data['BBmid'].iloc[i] / 2) &
                (self.data['maxterm'].iloc[i]) &
                (self.data['maxtermr'].iloc[i]) 
            ):
                self.data['Signal'].iloc[i] = -1
        return self.data
